...
| resolution | string | ❌ | 1k | `1k`, `2k`, `4k` | 清晰度档位，与 `aspect_ratio` 组合映射官方像素 |
...
### 2.3.1 GPT Image 2 官方尺寸映射表（项目标准）

> 实现位置：`frontend/src/constants/imageSizeSpec.ts`、`backend/app/core/image_size_spec.py`  
> 前端仅展示下表组合；后端 `/api/tasks/generate` 会校验 `size` 或 `aspect_ratio+resolution` 必须命中下表。

| aspect_ratio | 1k (size) | 2k (size) | 4k (size) |
|--------------|-----------|-----------|-----------|
