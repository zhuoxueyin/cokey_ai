...
export default function ChannelBindingEditor({
  ...
  const channelFor = (channelCode: string) =>
    channelList.find((c) => c.channel_code === channelCode)
  ...
  const handleChannelChange = (idx: number, channelCode: string) => {
    const b = bindings[idx]
    const channel = channelFor(channelCode)
    updateBinding(idx, {
      channel_code: channelCode,
      mode_profiles: pruneIncompatibleModeProfiles(
        b.mode_profiles,
        profileOptions,
        channel,
        b.channel_model_id,
  ...
  const handleChannelModelChange = (idx: number, channelModelId: string) => {
    const b = bindings[idx]
    const channel = channelFor(b.channel_code)
    updateBinding(idx, {
            ...
            <Row gutter={12}>
              <Col span={8}>
                ...
                <Select
                  ...
                  onChange={(val) => handleChannelChange(idx, val)}
                  style={{ width: '100%' }}
                >
                  {channelList.map((c) => (
              ...
              <Col span={8}>
                ...
                <Input
                  ...
                  onChange={(e) => handleChannelModelChange(idx, e.target.value)}
                />
              </Col>
              <Col span={4}>
              ...
              <div style={{ fontSize: 12, fontWeight: 600, marginBottom: 8, color: '#444' }}>
                ...
                {channel?.channel_provider ? (
                  <span style={{ fontWeight: 400, color: '#888', marginLeft: 8 }}>
                    按渠道端点 + 模型族筛选（{channel.channel_name || channel.channel_code}）
                  </span>
                ) : null}
              </div>
              {modes.map((mode) => {
                      ...
                      <Select
                        ...
                        placeholder={
                          options.length
                            ? '选择协议画像 profile_id'
                            : b.channel_model_id
                              ? '无匹配画像：请检查渠道端点或 channel_model_id'
                              : '请先填写渠道模型 ID，或检查渠道 endpoints'
                        }
                        style={{ width: '100%' }}
